...
double
best_nearby(double * delta, double * point, double prevbest, int nvars, HOOKE_FUNC func, void *param)
...
int
hooke(int nvars, double * startpt, double * endpt, double rho, double epsilon, int itermax, HOOKE_FUNC func, void *param,
     ...
     //	   double	   rho, epsilon;
{
	   ...
	   steplength = rho;
	   iters = 0;
	   fbefore = (*func)(newx, nvars, param);
	   newf = fbefore;
	   while ((iters < itermax) && (steplength > epsilon)) {

		   if(progress_func) {
			   if((* progress_func)(param, iters, newf)) break;
		   }
		   ...
		   newf = best_nearby(delta, newx, fbefore, nvars, func, param);
...
main()
{
	   ...
	   rho = RHO_BEGIN;
	   epsilon = EPSMIN;
	   jj = hooke(nvars, startpt, endpt, rho, epsilon, itermax, ff, NULL);
